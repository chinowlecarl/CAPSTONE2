...
export default function ManageProducts({ setPage }) {
  ...
  const openAdd = () => {
    setForm({ title:"", description:"", price:"", discount:"0", category:"", stock:"", featured:false, image_url:"" });
    setEditP(null); setModal(true);
  };
  const openEdit = (p) => {
    setForm({ title:p.title, description:p.description||"", price:p.price, discount:p.discount, category:p.category, stock:p.stock, featured:!!p.featured, image_url:p.image_url||"" });
    setEditP(p); setModal(true);
  };

  const save = async () => {
    try {
      const body = { ...form, price:+form.price, discount:+form.discount, stock:+form.stock };
      if (editP) {
                ...
                {filtered.map((p,i)=>(
                  <tr key={p.id} style={{ borderBottom:"1px solid #fce7f3", background:i%2===0?"#fff":"#fff8fb" }}
                    ...
                    <td style={{ padding:"10px 14px" }}>
                      <img src={p.image_url} alt={p.title} style={{ width:52, height:52, objectFit:"cover", borderRadius:8, border:"1px solid #fce7f3" }}
          ...
          <div style={{ background:"#fff", borderRadius:20, width:"100%", maxWidth:560, maxHeight:"92vh", overflowY:"auto", boxShadow:"0 20px 60px rgba(0,0,0,0.3)" }}>
            ...
            <div style={{ padding:24, display:"flex", flexDirection:"column", gap:14 }}>
              ...
              <div>
                ...
                <input value={form.image_url} onChange={sf("image_url")} placeholder="https://..."
                  style={inputStyle}
                  onFocus={(e)=>(e.target.style.borderColor="#db2777")} onBlur={(e)=>(e.target.style.borderColor="#fce7f3")} />
